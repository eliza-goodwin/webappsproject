...
#route for OPTIONS
@app.route("/backend/plans/<int:plan_id>", methods=["OPTIONS"])
...
#route for GET
@app.route("/backend/plans", methods=["GET"])
def retrieve_plans():
...
#route for POST
@app.route("/backend/plans", methods=["POST"])
def create_plan():
    db = PlansDB("plans_db.db")
...
#route for PUT
@app.route("/backend/plans/<int:plans_id>", methods=["PUT"])
...
@app.route("/backend/plans/<int:plans_id>", methods=["DELETE"])
...
@app.route("/backend/plans/<int:plans_id>", methods=["GET"])
...
def run():
    app.run(port=8080, host='0.0.0.0') # Ensure the server runs on port 8080
